...
1. **Phase 1（今回）**: docker-compose で小規模に動かす（オンプレ・外部依存ゼロ）
2. **Phase 2**: 手軽さ・安定性を求めて Cloudflare（サーバーレス）へ
3. **Phase 3**: スケール / 学習目的でオンプレ k8s へ
...
| Auth（後） | OAuth ライブラリ（Better Auth 等） | 同左 or Cloudflare Access | 同左 |
